...
def classification_data() -> Tuple[np.ndarray, np.ndarray]:
    ...
    y1 = np.array([np.array([0 for _ in range(N_class1)])])

    x2_mean = np.array([0.0, 0.0])
    x2_cov = np.array([[1.0, 0.0], [0.0, 3.0]])
    x2 = np.random.multivariate_normal(mean=x1_mean, cov=x1_cov, size=N_class2)
    y2 = np.array([np.array([1 for _ in range(N_class2)])])
...
def regression_data() -> Tuple[np.ndarray, np.ndarray]:
    ...
    y = f(x) + np.random.normal(scale=2.0, size=N)
    return (x, y)
